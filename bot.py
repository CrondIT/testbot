...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = """
                Привет! Я Нейросеть МЛСПО 🤖
        Я помогаю быстро: отвечаю на вопросы,
        ищу информацию в интернете, разбираю файлы и делаю изображения.

        Как пользоваться (очень просто)
        1. Выбери, что тебе нужно, и нажми команду ниже.
        2. Напиши задачу обычными словами.
        3. Я пришлю готовый результат. Если не хватит данных — уточню.

        Команды меню

        🧠 /ai — задать вопрос, попросить текст или поиск в интернете
        Примеры:
        • «Объясни простыми словами, что такое…»
        • «Напиши объявление/пост про…»
        • «Найди свежую информацию про…»

        📎 /ai_file — анализ файла
        Отправь файл (PDF/Word/Excel и т.п.) и напиши, что сделать:
        • «Сделай краткое содержание»
        • «Выдели главное и риски»
        • «Ответь на вопросы по документу»

        🖼️ /ai_edit — изображения
        Напиши, что нужно, или пришли картинку и скажи, что изменить:
        • «Сделай баннер с текстом…»
        • «Убери фон»
        • «Сделай в другом стиле»

        💳 /billing — тарифы и оплата
        Посмотреть цены, лимиты и статус подписки.

        🔒 /privacy — конфиденциальность
        Как обрабатываются данные и файлы.

        📄 /offer — условия использования (оферта)
        Правила сервиса, оплата и другие условия.

        Важный совет

        Чем точнее запрос — тем лучше ответ.
        Можно писать так: «Сделай [что] для [кого]
        в стиле [каком], объём [сколько]».

        Напиши, что нужно сделать — и начнём 👇
        """
...
async def ai_edit_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = """
        ...
        ИЛИ
        1. Отправьте несколько изображений (до 6)
