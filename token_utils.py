...
class TokenCounter:
    ...
    def count_openai_tokens(self, text: str, model: str = "gpt-4o-mini") -> int:
        """
    ...
    def count_openai_messages_tokens(self, messages: List[Dict[str, str]], model: str = "gpt-4o-mini") -> int:
        """
    ...
    def estimate_gemini_tokens(self, text: str) -> int:
        """
        Estimate tokens for Gemini models (Google doesn't provide exact count without API call)
...
def truncate_messages_for_token_limit(messages: List[Dict[str, str]], model: str = "gpt-4o-mini", 
                                    max_tokens: int = None, reserve_tokens: int = 1000) -> List[Dict[str, str]]:
    """
    ...
    # Keep the system message if present, and remove oldest user/assistant pairs
    if not messages:
        return []
    ...
    system_tokens = 0
    if system_message:
        system_tokens = token_counter.count_openai_messages_tokens([system_message], model)
    ...
    for msg in reversed(conversation_messages):
        ...
        if current_tokens + msg_tokens <= available_for_conversation:
            truncated_conversation.insert(0, msg)  # Insert at beginning to maintain order
...
def check_token_usage(messages: List[Dict[str, str]], model: str = "gpt-4o-mini", 
                     max_tokens: int = None, reserve_tokens: int = 1000) -> Dict[str, Any]:
    """
